...
if (count($_POST))
{
    if ($_POST['action'] === 'edit')
    {
        $id = (int) $_POST['id'];
        $title = $_POST['title'];
        $query = "UPDATE `apartment_types` SET `title` = '{$title}' WHERE `id` = '$id'";
...
<head>
    ...
    <title>Редактирование типа недвижимости: <?= $type['title'] ?></title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Редактирование типа недвижимости</h1>
                    <form action="" method="post">
                        <input type="hidden" name="action" value="edit"/>
                        <input type="hidden" name="id" value="<?= $type['id'] ?>"/>
                        <div class="form-group input-group">
                            <span class="input-group-addon"><i class="fa fa-tasks"></i></span>
                            <input type="text" placeholder="Название" class="form-control" name="title" value="<?= $type['title'] ?>">
                        </div>
                        <button class="btn btn-success" type="submit"><i class="fa fa-check"></i> Сохранить</button>
                        <a class="btn btn-default" href="view_type.php?id=<?= $type['id'] ?>">Отмена</a>
                    </form>
                </div>
                <!-- /.col-lg-12 -->
